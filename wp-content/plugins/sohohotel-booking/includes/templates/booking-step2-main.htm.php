<form name="bookroom" class="booking-form-data">
	...
	$fully_booked_rooms_db = sh_get_all_booked_room_ids($_SESSION['sh_booking_data']['check_in'],$_SESSION['sh_booking_data']['check_out'],true);
	
	// Get IDs of all bookings for remaining room check
	$booked_ids_array = sh_get_booked_room_ids_db($_SESSION['sh_booking_data']['check_in'],$_SESSION['sh_booking_data']['check_out']);
			...
			<div class="booking-room-wrapper clearfix <?php if ($occupancy_hide == true) {echo 'occupancy-hide';} ?>">
				...
				<?php if( has_post_thumbnail() ) { ?>
					...
					<a target="_blank" href="<?php esc_url(the_permalink()); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
						...
						<?php echo '<img src="' . $src[0] . '" alt="" class="booking-room-image" />'; ?>
					</a>
