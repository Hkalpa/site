...
-------------------------------------------------------------------------------- */
function sh_get_booked_room_ids_db($check_in,$check_out) {
		...
		while($wp_query->have_posts()) :	
			...
			if ($booking_meta["booking_status"] == '2') {
				...
				foreach($room_booking_data_decoded as $key => $val) {
					...
					$check_in_db = $val['check_in'];
					$check_out_db = date('Y-m-d', strtotime($val['check_out'] . ' -1 day'));
	...
	// Number of bookings for each ID stored here 
	return $booked_ids;
...
function sh_get_fully_booked_room_ids($booked_ids) {
	...
	$frequencies = array_count_values($booked_ids);
...
function sh_get_booked_room_ids_session($check_in,$check_out) {
	...
	if( !empty($_SESSION['sh_booking_data']) ) {
		...
		foreach (range(1, $_SESSION['sh_booking_data']["book_room"]) as $r) {
			$booked_ids[] = $_SESSION['sh_booking_data']["room_" . $r]["room_id"];
		}
...
-------------------------------------------------------------------------------- */
function sh_get_all_booked_room_ids($check_in,$check_out,$check_session) {
	
	// Get booked room IDs from the database
	$db_booked_ids = sh_get_booked_room_ids_db($check_in,$check_out);
		...
		<?php while($wp_query_2->have_posts()) :
			...
			$wp_query_2->the_post(); 
				...
				$accommodation_meta = json_decode( get_post_meta($post->ID,'_accommodation_meta',TRUE), true ); ?>
					...
					// Use nightly pricing
					if ( $number_of_nights < $weekly_price_limit ) {
					...
					// Use weekly pricing
					} elseif ( $number_of_nights >= $weekly_price_limit && $number_of_nights < $monthly_price_limit ) {
					...
					// Use monthly pricing
					} elseif ( $number_of_nights >= $monthly_price_limit ) {
		...
		if ($guest_num > 1 ) {
			foreach (range(1, $guest_num - 1) as $r) {
				...
				if( strlen($price_data[$seasonal_key]["price"][$r]["price_" . $guest_type . "_weekdays"]) != 0 ) {
					...
					if( strlen($price_data[$seasonal_key]["price"]) != 0 ) {
...
function sh_store_booking_db($data_post,$data_session,$data_ical) {
	...
	if ( $sohohotel_data["manual_booking_confirmation"] == '0' ) {
	...
	if ( $sohohotel_data["do_not_record_bookings"] == '0' ) {
...
-------------------------------------------------------------------------------- */
function sh_send_booking_emails($data,$booking_id) {
	...
	$email_content .= '<li>' . esc_html__('Check In', 'sohohotel_booking') . ': ' . sh_display_formatted_date($sh_booking_data["check_in"]) . '</li>';
	$email_content .= '<li>' . esc_html__('Check Out', 'sohohotel_booking') . ': ' . sh_display_formatted_date($sh_booking_data["check_out"]) . '</li>';
	$email_content .= '</ul>';
	
	// Room Details
	foreach (range(1, $sh_booking_data["book_room"]) as $r) {
			
		$email_content .= '<h3>' . esc_html__('Room', 'sohohotel_booking') . ' ' . $r . '</h3>';
		$email_content .= '<ul>';
		$email_content .= '<li>' . esc_html__('Room Name', 'sohohotel_booking') . ': ' . $sh_booking_data["room_" . $r ]["room_name"] . '</li>';
		$email_content .= '<li>' . esc_html__('Adults', 'sohohotel_booking') . ': ' . $sh_booking_data["room_" . $r ]["adults"] . '</li>';
		$email_content .= '<li>' . esc_html__('Children', 'sohohotel_booking') . ': ' . $sh_booking_data["room_" . $r ]["children"] . '</li>';
		$email_content .= '<li>' . esc_html__('Price', 'sohohotel_booking') . ': ' . sh_get_price($sh_booking_data["room_" . $r ]["room_price"]) . '</li>';
		$email_content .= '</ul>';
		
	}
	
	// Service Details
	if ( !empty( $_SESSION['sh_booking_data']["services"]["service_1"]["service_name"] ) ) {
		
		$email_content .= '<h3>' . esc_html__('Services', 'sohohotel_booking') . '</h3>';	
		foreach ($_SESSION['sh_booking_data']["services"] as $key => $value) {	
			$email_content .= '<li>' . $value["service_name"] . ': ' . sh_get_price($value["service_price"]) . '</li>';
		}
	...
	$email_content .= '<ul>';
	foreach ( $data["_booking_meta"] as $key => $val ) {
		$new_key1 = str_replace('_', ' ', $key);
		$new_key2 = ucwords(strtolower($new_key1));
		$email_content .= '<li>' . $new_key2 . ': ' . $val . '</li>';
	}
	$email_content .= '</ul>';
	...
	$email_content .= '<h3>' . esc_html__('Booking Details', 'sohohotel_booking') . '</h3>';
	$email_content .= '<ul>';
	$email_content .= '<li>' . esc_html__('Selected Payment Method', 'sohohotel_booking') . ': ' . $data["payment_method"] . '</li>';
	$email_content .= '<li>' . esc_html__('Amount Paid', 'sohohotel_booking') . ': ' . sh_get_price('0') . '</li>';
	$email_content .= '<li>' . esc_html__('Amount Due', 'sohohotel_booking') . ': ' . sh_get_price($sh_booking_data["total_booking_price"]) . '</li>';
	$email_content .= '</ul>';
	
	// Guest Email
	$guest_recipient_email = $data["_booking_meta"]["email_address"];
	$guest_email_subject = $sohohotel_data["booking_email_subject_guest"];
	...
	$admin_email_subject = $sohohotel_data["booking_email_subject_admin"];
...
function sohohotel_stripe_payment($data) {
	...
	{
		$amount_cents = str_replace(".","",$data["price"]);  // Chargeble amount
		...
		if ( $result == 'success' ) {
			...
			$update_data["payment_method"] = esc_html__('Stripe', 'sohohotel_booking');
			...
			$email_content .= '<h3>' . esc_html__('Payment Details', 'sohohotel_booking') . '</h3>';
			$email_content .= '<ul>';
			$email_content .= '<li>' . esc_html__('Amount Paid', 'sohohotel_booking') . ': ' . $charge["amount"] . ' (' . $charge["currency"] . ')</li>';	
			$email_content .= '</ul>';
			...
			wp_mail($update_data["email_address"],esc_html__('Credit Card Payment Details', 'sohohotel_booking'),$email_content,$admin_email_headers);
	...
	if ($wp_query->have_posts()) :
		while($wp_query->have_posts()) :
			$wp_query->the_post();
				...
				if ( sh_get_date_range_overlap($from,$to,$blocked_dates_meta["from"],$blocked_dates_meta["to"]) == true ) {
					
					// Check if blocked day occurs in requested booking dates
					foreach( sh_get_date_range_array($from,$to) as $date_key => $date) {
...
function sh_frontend_availability_calendar($room_id,$m,$y) {
	...
	echo '<button type="button" class="calendar_button_prev">&lt; Prev</button>';
	echo '<button type="button" class="calendar_button_current">Current Month</button>';
	echo '<button type="button" class="calendar_button_next">Next &gt;</button>';
	...
	foreach( $list as $key => $val) {
		...
		if ( $check_room == 'all' ) {
			...
			if ( sh_array_match($sh_get_all_room_ids,sh_get_all_booked_room_ids(date('Y-m-d', $val),date('Y-m-d', $val),false)) == true ) {
		...
		// Check availability of a specific room
			...
			if (in_array($check_room, sh_get_all_booked_room_ids(date('Y-m-d', $val),date('Y-m-d', $val),false) )) {
			  $availability_class = 'sh_cal_unavailable';
			} else {
				$availability_class = 'sh_cal_available';
			}
...
function sh_get_import_ical() {
	...
	if ( in_array($room_id,$sh_get_all_room_ids,true) ) {
		...
		foreach ($events as $event) {
			...
			// Check if feed is from AirBNB
			if (strpos($_GET['feed_url'], 'airbnb.') !== false) {
			    $airbnb_feed = true;
			} else {
				$airbnb_feed = false;
			}
			
			// If feed is from AirBNB use "SUMMARY" instead of "UID" to sync booking because AirBNB changes the UID each time the feed is accessed
			if ($airbnb_feed == true) {
				$unique_booking_id = str_replace('\\', '', $event['SUMMARY']);
				$unique_booking_id_array = $booking_summaries;
			} else {
				$unique_booking_id = $event['UID'];
				$unique_booking_id_array = $booking_uids;
			}
			
			// Booking already exists - edit it if data is different
			if ( in_array($unique_booking_id,$unique_booking_id_array,true) ) {
				
				$booking_meta = get_post_meta($unique_booking_id,'_booking_meta',TRUE);
				$room_booking_data = $booking_meta['save_rooms'];
				$room_booking_data_decoded = json_decode($room_booking_data, true);
				
				// If all data is the same do nothing
				if($booking_session["check_in"] == $room_booking_data_decoded["Room 1"]['check_in'] && $booking_session["check_out"] == $room_booking_data_decoded["Room 1"]['check_out'] && $booking_session["room_1"]["room_id"] == $room_booking_data_decoded["Room 1"]['room_type']) {
	
				// If some data has changed edit the booking
				} else {
					
					$updated_booking = array();
					$updated_booking['save_rooms'] = '{"Room 1":{"check_in":"' . $booking_session["check_in"] . '","check_out":"' . $booking_session["check_out"] . '","room_type":"' . $booking_session["room_1"]["room_id"] . '","adults":"0","children":"0"}}';	
					$updated_booking['ical_data'] = $event;

					update_post_meta( $unique_booking_id, '_booking_meta', $updated_booking );
					
				}
				
			// Booking does not already exist so insert new booking
			} else {
				
				// Check availability
				
				// Room is unavailable
				if (in_array($booking_session["room_1"]["room_id"], sh_get_all_booked_room_ids($booking_session["check_in"],$booking_session["check_out"],true))) {
		...
		while($wp_query->have_posts()) :
			...
			if(!empty($accommodation_meta["ical_feed"])) {
				...
				foreach($ical_array as $key => $val) {
					...
					$ical_url = get_site_url() . '/?sh_ical=import&room_id=' . get_the_ID() . '&feed_url=' . $val;
