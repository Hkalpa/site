...
------------------------------------------------ */
	...
	function __construct() {
		...
		parent::__construct(false, $name = esc_html__('Soho Hotel Recent Posts','sohohotel'), array(
			'description' => esc_html__('Display Recent Posts','sohohotel')
		));
...
------------------------------------------------ */
	...
	function widget( $args, $instance ) {
				...
				<div class="news-widget-wrapper clearfix">	
					...
					<?php if( has_post_thumbnail() ) { ?>
						...
						<a href="<?php esc_url(the_permalink()); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
							<?php $src = wp_get_attachment_image_src( get_post_thumbnail_id($post_query->ID), 'sohohotel-image-style3' ); ?>
		...
		<?php echo wp_kses($after_widget,$sohohotel_allowed_html_array);
...
------------------------------------------------ */
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'post_limit' )); ?>"><?php esc_html_e('Post Limit:', 'sohohotel') ?></label>
